...
async def confirm_use_ticket_keyboard(ticket_id):
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text='✅ Отметить как использованный',
                             callback_data=f'use_ticket_{ticket_id}')
    ]])
...
async def all_groups_keyboard():
    ...
    buttons.append([InlineKeyboardButton(
        text='Не от групп', callback_data='no_each_one')])
...
async def get_subscription_keyboard_with_link(channels):
    ...
    if len(channels) == 1:
        buttons.append([InlineKeyboardButton(
            text='📢 Перейти в канал', url=f'https://t.me/{channels[0]}')])
    else:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text='✏️ Редактировать',
                                 callback_data=f'edit_concert_{concert_id}'),
            InlineKeyboardButton(
                text='📢 Рассылка', callback_data=f'broadcast_concert_{concert_id}')
        ],
        [
            InlineKeyboardButton(
                text=act_inact_text, callback_data=f'deactivate_concert_{concert_id}'),
            InlineKeyboardButton(text='📋 Список концертов',
                                 callback_data='list_concerts')
        ]
    ])
...
async def get_edit_concert_keyboard(concert_id):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text='📝 Название', callback_data=f'edit_name_{concert_id}'),
            InlineKeyboardButton(
                text='📄 Описание', callback_data=f'edit_description_{concert_id}')
        ],
        [
            InlineKeyboardButton(
                text='📅 Дата', callback_data=f'edit_date_{concert_id}'),
            InlineKeyboardButton(
                text='📍 Адрес', callback_data=f'edit_address_{concert_id}')
        ],
        [
            InlineKeyboardButton(
                text='🖼️ Фото', callback_data=f'edit_photos_{concert_id}')
        ],
        [
            InlineKeyboardButton(
                text='⬅️ Назад', callback_data=f'back_to_management_{concert_id}')
        ]
    ])
...
async def get_back_to_edit_keyboard(concert_id):
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text='⬅️ Назад', callback_data=f'back_to_edit_menu_{concert_id}'),
        InlineKeyboardButton(text='❌ Отмена редактирования',
                             callback_data=f'back_to_concert_card_{concert_id}'),
    ]])
